...
# 提取文档csv信息
documents = CSVLoader(file_path='./train_sent_emo.csv').load()
...
Conversation is historical conversation information. Context is the mood of the conversation similar to my utterance. You can choose whether to refer to context or not.
...
utterance is current utterance and emotion is its emotion label.
preUtterance: {preUtterance}
preUtterance is the last line of utterance where I want to identify emotions. 
...
This is my chat history which include utterance emotion, intentions and reactions.
Please explain what you think is the reason for this emotion.You can refer to intentions and reactions.
Just give a short answer
"""
# 初始化 三个Prompt 对象
output_parser = StrOutputParser()
prompt1 = PromptTemplate(template=template1, input_variables=["conversation", "context", "utterance"], output_parser=output_parser)
llm_chain1 = LLMChain(prompt=prompt1, llm=llm)
prompt2 = PromptTemplate(template=template2, input_variables=["utterance", "emotion", "preUtterance"], output_parser=output_parser)
llm_chain2 = LLMChain(prompt=prompt2, llm=llm)
prompt3 = PromptTemplate(template=template3, input_variables=["chat_history"], output_parser=output_parser)
...
for dialogue_id, data in data_by_dialogue_id.items():
    ...
    for item in data:
        ...
        answer1 = llm_chain1.predict(conversation=conversation, context=docs[0].page_content, utterance=utterance)
        answer2 = llm_chain2.predict(utterance=utterance, emotion=answer1, preUtterance=pre_utterance)
        chat_history = template2.format(utterance=utterance, emotion=answer1, preUtterance=pre_utterance) + answer2
        answer3 = llm_chain3.predict(chat_history=chat_history)
...
csv_columns = ['Utterance', 'Speaker', 'Emotion', 'Sentiment', 'Dialogue_ID', 'Generate_emotion', 'tips']
